...
interface List<t>{
   ...
   command void pushback(t input);
	...
	command t get(uint16_t position); //gets the value @ position
  command t* getAddress(uint16_t position); //gets the physical data address of entry in array
  command uint16_t getMaxSize();
}
